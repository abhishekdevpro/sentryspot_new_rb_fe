...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (orderId) {
      const verifyOrder = async () => {
        try {
          const response = await fetch(`https://api.sentryspot.co.uk/api/jobseeker/paypal/verify-order?orderid=${orderId}`);
          if (!response.ok) {
              ...
              <div className="absolute">
                <div className>
                  ...
                  <button
                    ...
                    className="sm:w-full lg:w-auto my-2 border rounded md py-4 px-8 text-center bg-indigo-600 text-[#003479] hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-700 focus:ring-opacity-50"
                  >
                    Take me there!
                  </button>
                </div>
  ...
  return (
    <>
      ...
      <button
        onClick={handleRedirect}
        className="mt-4 bg-blue-900 text-[#003479] px-4 py-2 rounded"
      >
        Go to AI Builder
      </button>
    </>
