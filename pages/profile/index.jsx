...
const ProfileForm = () => {
    ...
    const fetchData = async () => {
      try { 
        ...
        if (userProfileResponse.data.status === 'success') {
          ...
          // Fetch countries
          const countriesResponse = await axios.get('https://api.sentryspot.co.uk/api/jobseeker/countries');
  ...
  useEffect(() => {
    const fetchStates = async () => {
      if (formData.country_id) {
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get(`https://api.sentryspot.co.uk/api/jobseeker/stats/${formData.country_id}`, {
            headers: {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      if (formData.state_id) {
        ...
        try {
          const citiesResponse = await axios.get(`https://api.sentryspot.co.uk/api/jobseeker/cities/${formData.state_id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch('https://api.sentryspot.co.uk/api/jobseeker/user-profile', formDataToSend, {
        headers: {
