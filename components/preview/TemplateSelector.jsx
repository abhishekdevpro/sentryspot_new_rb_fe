...
const TemplateSelector = ({ selectedTemplate, setSelectedTemplate }) => {
  ...
  return (
    <div>
      <button
        onClick={openModal}
        className="rounded-lg border-2 m-2 border-[#003479] px-5 py-2 font-bold bg-white text-[#003479] "
      >
        Select Template
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-75 p-4">
          <div className="bg-white rounded-lg p-5 w-full max-w-3xl md:max-w-4xl relative">
            <h2 className="text-lg font-bold mb-4 text-center border rounded-3xl py-2 text-[#fff] bg-[#003479]">
              Select a Template
            </h2>
            ...
            <div className="relative flex items-center">
              <button
                ...
                className="absolute -left-4 md:-left-10 top-1/2 transform -translate-y-1/2 bg-[#003479] text-[#fff] p-2 md:px-6 md:py-4 border-4 border-white rounded-full"
              >
                &lt;
              </button>
                  ...
                  {getDisplayedTemplates().map((template, index) => (
                    <div
                      ...
                      className={`flex-none w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/3 px-2 ${
                        template.key === templates[currentIndex].key
                          ? "border-4 p-0 rounded-lg border-purple-500 shadow-3xl"
                          : ""
                      }`}
              ...
              <button
                onClick={goToNext}
                className="absolute -right-4 md:-right-10 top-1/2 transform -translate-y-1/2 bg-[#003479] text-[#fff] p-2 md:px-6 md:py-4 border-4 border-white rounded-full"
              >
                &gt;
              </button>
            </div>
            ...
            <button
              onClick={closeModal}
              className="mt-4 w-full sm:w-auto px-5 py-2 bg-red-500 text-[#fff] font-bold rounded-lg"
            >
              Close
            </button>
          </div>
