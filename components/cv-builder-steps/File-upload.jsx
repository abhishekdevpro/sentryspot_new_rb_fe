  ...
  const handleUpload = async (file) => {
    ...
    try {
      const response = await axios.post(
        `https://api.sentryspot.co.uk/api/jobseeker/resume-upload/12345`,
        formData,
        {
          headers: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
      <div className="text-center">
        ...
        <p className="mt-3 text-gray-600">We accept PDF files for better parsing accuracy</p>
      </div>
      ...
      >
        ...
        {isUploading ? (
          ...
          <>
            <p className="text-xl font-medium mb-4">Drag and drop your resume here</p>
            <button className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
              Select PDF File
            </button>
            <p className="mt-4 text-sm text-gray-500">Maximum file size: 10MB</p>
          </>
        )}
      </div>
